...
float BloodMoonFactor( float ph )
{
	...
	return skymath::clampf01( 1.0f - d * ( 1.0f / 0.08f ) );
}
...
void SkyRenderer::RegisterDevCvars()
{
	...
	// Blood-moon tint is DISABLED by default (csz_bloodmoon 0). BloodMoonFactor
	// and the whole shader blood-moon path stay intact -- they are just fed 0.
	// A future M5 blood-moon server mode re-enables it via "csz_bloodmoon 1".
...
void SkyRenderer::DrawSky( const ViewSetup &view )
{
	...
	float moonColor[3] = { 0.90f, 0.93f, 1.00f };	// near-white disc core (luma ~0.93), faint cool tint at the edges
	...
	// Disabled by default -- re-enable via "csz_bloodmoon 1" (future M5 blood-moon mode).
	float bloodMoon = ( s_bloodMoonCvar != NULL && s_bloodMoonCvar->value != 0.0f ) ? BloodMoonFactor( phase ) : 0.0f;
	...
	glUniform1f( s_sky.uMoonHalo, 0.9f );
...
void SkyRenderer::PublishLighting( AmbienceParams &amb, float phase )
{
	...
	float dayR = 1.00f, dayG = 1.00f, dayB = 1.00f;			// daylight @1.00: full neutral (reached only in the last 5s)
