...
static const char *kWaterVs =
	...
	"layout(location=3) in vec2 a_faceCenter;\n"	// source face world-XY centroid (rain ring center)
	"uniform mat4 u_viewProj;\n"
	"out vec2 v_uv;\n"
	"out vec3 v_normal;\n"
	"out vec3 v_worldPos;\n"
	"out vec2 v_faceCenter;\n"
	"void main()\n"
	"{\n"
	"	v_uv = a_uv;\n"
	"	v_normal = a_normal;\n"
	"	v_worldPos = a_pos;\n"
	"	v_faceCenter = a_faceCenter;\n"
...
static const char *kWaterFs =
	...
	"in vec3 v_normal;\n"
	"in vec3 v_worldPos;\n"
	"in vec2 v_faceCenter;\n"		// source face world-XY centroid (rain ring center)
	"uniform sampler2D u_tex;\n"		// diffuse, TMU 0
	"uniform sampler2D u_reflTex;\n"	// REAL planar reflection (mirrored scene), TMU 1
	"uniform sampler2D u_refrTex;\n"	// REAL refraction (scene-behind color copy), TMU 2
	"uniform int u_reflectOn;\n"		// 1 = sample real refl/refr; 0 = analytic-sky fallback
	"uniform vec2 u_viewport;\n"		// (w,h) so gl_FragCoord.xy -> screen UV
	...
	"		// Concentric rain rings. Radius AND outward direction are measured from\n"
	"		// the SAME center -- the source face centroid (baked per-vertex into\n"
	"		// a_faceCenter) -- in WORLD units, so the rings are genuinely concentric\n"
	"		// and coherent instead of the fine grid-cell chop the old fract() tiling\n"
	"		// produced (radius was on a 25u tile, direction effectively per ~1u).\n"
	"		// A slow per-face rotation of the center keeps successive ring fronts\n"
	"		// from all snapping to one phase across the surface.\n"
	"		float wob   = u_time * 0.6;\n"
	"		vec2  cen   = v_faceCenter + vec2( cos( wob ), sin( wob ) ) * 6.0;\n"
	"		vec2  cc    = v_worldPos.xy - cen;\n"		// world-space offset from ring center
	"		float r     = length( cc );\n"
	"		// ~24-unit wavelength rings expanding outward at u_time*7; fade with radius.\n"
	"		float ring  = sin( r * 0.26 - u_time * 7.0 );\n"
	"		float fade  = exp( -r * 0.012 );\n"		// rings die off ~80u from impact
	"		grad += normalize( cc + 1e-4 ) * ( ring * fade * 0.06 * wetness );\n"
	"	}\n"
	...
	"	// reflection feel) without over-roughening the specular/Fresnel below. A\n"
	"	// second, finer ripple octave jitters the reflected ray's up-component so the\n"
	"	// mirrored sky/horizon line breaks into shifting bands (a screen-space-style\n"
	"	// perturbation of the analytic sky gradient -- no cubemap, no FBO).\n"
	"	vec3 Nref = normalize( N - vec3( grad, 0.0 ) * 1.6 );\n"
	"	vec3 R = reflect( -V, Nref );\n"
	"	float band = sin( dot( v_worldPos.xy, vec2( 0.11, -0.07 ) ) + u_time * 1.7 )\n"
	"	           + 0.6 * sin( dot( v_worldPos.xy, vec2( -0.05, 0.13 ) ) - u_time * 2.3 );\n"
	"	R.z = clamp( R.z + band * ( 0.05 + length( grad ) * 0.30 ), -1.0, 1.0 );\n"
	"	R = normalize( R );\n"
	"\n"
	"	float viewDepth = max( dot( normalize( v_normal ), V ), 0.05 );\n"		// 1 top-down .. ~0 grazing
	"	vec3 sunCol = ( day > 0.0 ) ? mix( u_moonColor, vec3( 1.0, 0.96, 0.85 ), day ) : u_moonColor;\n"
	"\n"
	"	// Screen-space UV from window coords (for the REAL refl/refr textures).\n"
	"	vec2 screenUV = gl_FragCoord.xy / max( u_viewport, vec2( 1.0 ) );\n"
	"	// Ripple distortion of the screen-space lookups: the animated normal\n"
	"	// gradient pushes the sample sideways so the mirrored scene and the\n"
	"	// scene-behind both wobble with the waves.\n"
	"	vec2 sdist = grad * vec2( 0.045, 0.045 );\n"
	"\n"
	"	vec3 body;\n"
	"	vec3 refl;\n"
	"	float f0;\n"
	"	if( u_reflectOn == 1 )\n"
	"	{\n"
	"		// 2a/3a. REAL render-to-texture path (Source-style planar reflection +\n"
	"		//        framebuffer-copy refraction). reflTex = the mirrored scene\n"
	"		//        rendered into a half-res FBO; refrTex = a copy of the main\n"
	"		//        color buffer (scene BEHIND the water). Both sampled at the\n"
	"		//        fragment screen UV, distorted by the ripple gradient.\n"
	"		vec3 reflScene = texture( u_reflTex, clamp( screenUV + sdist, 0.001, 0.999 ) ).rgb;\n"
	"		vec3 refrScene = texture( u_refrTex, clamp( screenUV + sdist * 0.6, 0.001, 0.999 ) ).rgb;\n"
	"		// Tint the refracted scene toward deep water (light absorbs with depth\n"
	"		// through the column) so the body still reads as water, not glass.\n"
	"		vec3 deepDay   = vec3( 0.05, 0.22, 0.28 );\n"
	"		vec3 deepNight = vec3( 0.01, 0.05, 0.09 );\n"
	"		vec3 deepTint = mix( deepNight, deepDay, day );\n"
	"		float absorb = mix( 0.45, 0.18, viewDepth );\n"			// grazing = longer path = more absorb
	"		body = mix( refrScene * vec3( 0.55, 0.82, 0.92 ), deepTint, absorb );\n"
	"		refl = reflScene;\n"
	"		// Reflection is REAL now -> drop f0 toward physical water (~0.02), keep a\n"
	"		// small floor so the mirror still reads from above.\n"
	"		f0 = 0.04;\n"
	"	}\n"
	"	else\n"
	"	{\n"
	"		// 2b/3b. ANALYTIC FALLBACK (csz_water_reflect 0 or GL failure): the old\n"
	"		//        in-shader sky gradient + diffuse-UV-warp. Kept byte-identical so\n"
	"		//        the toggle is a true A/B.\n"
	"		float refrAmt = mix( 1.6, 0.4, viewDepth );\n"
	"		vec2 warp = grad * 0.5 * refrAmt;\n"
	"		vec3 tex = texture( u_tex, v_uv + warp ).rgb;\n"
	"		vec3 deepDay   = vec3( 0.03, 0.20, 0.26 );\n"
	"		vec3 deepNight = vec3( 0.01, 0.05, 0.09 );\n"
	"		vec3 deep = mix( deepNight, deepDay, day );\n"
	"		float bodyMix = mix( 0.34, 0.22, viewDepth );\n"
	"		body = mix( deep, tex * vec3( 0.40, 0.78, 0.92 ), bodyMix );\n"
	"		refl = skyReflect( R, u_moonDir, sunCol, u_phase );\n"
	"		f0 = 0.25;\n"
	"	}\n"
	"\n"
	"	// 3. FRESNEL: grazing angles show reflection, top-down shows the body.\n"
	"	float graze = 1.0 - max( dot( N, V ), 0.0 );\n"
	"	float fres = f0 + ( 1.0 - f0 ) * pow( graze, 3.5 );\n"
	"	vec3 col = mix( body, refl, clamp( fres, 0.0, 1.0 ) );\n"
	"	// Grazing-edge brightening: a bright wet-mirror rim toward the far edge so\n"
	"	// the water surface is unmistakable at shallow view angles. Softer when the\n"
	"	// real mirror is active (it already carries true scene brightness).\n"
	"	col += refl * pow( graze, 2.5 ) * ( ( u_reflectOn == 1 ) ? 0.35 : 0.80 );\n"
	"\n"
