...
static const char *kWaterFs =
	...
	"	vec2 grad = vec2( 0.0 );\n"
	"	float h = 0.0;\n"
	"	h += waveLayer( p, normalize( vec2(  1.0,  0.30 ) ), 1.00, 1.20, 0.060, u_time, grad );\n"
	"	h += waveLayer( p, normalize( vec2( -0.40, 1.0  ) ), 1.70, 0.90, 0.038, u_time, grad );\n"
	...
	"	vec3 deepDay   = vec3( 0.04, 0.16, 0.18 );\n"
	"	vec3 deepNight = vec3( 0.01, 0.04, 0.07 );\n"
	"	vec3 deep = mix( deepNight, deepDay, day );\n"
	"	vec3 body = mix( deep, tex * vec3( 0.55, 0.85, 0.95 ), 0.45 );\n"
	"\n"
	...
	"	float f0 = 0.02;\n"							// water normal-incidence reflectance
	"	float graze = 1.0 - max( dot( N, V ), 0.0 );\n"
	"	float fres = f0 + ( 1.0 - f0 ) * pow( graze, 5.0 );\n"
	"	vec3 col = mix( body, refl, clamp( fres, 0.0, 1.0 ) );\n"
	"	// Grazing-edge brightening: the reflection lifts toward the sky color at the\n"
	"	// shallowest view angles, giving the bright wet-mirror rim along the far edge.\n"
	"	col += refl * pow( graze, 4.0 ) * 0.25;\n"
	"\n"
	...
	"	float shin = mix( 220.0, 60.0, wetness );\n"			// rain -> broader, rougher
	"	float spec = pow( max( dot( N, H ), 0.0 ), shin );\n"
	"	col += sunCol * spec * ( 1.2 + 0.6 * day );\n"
	"\n"
