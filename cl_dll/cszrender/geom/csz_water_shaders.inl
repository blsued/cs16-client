...
static const char *kWaterFs =
	...
	"	R = normalize( R );\n"
	"\n"
	"	// 2. BODY (refraction): the diffuse tex is sampled through a REFRACTED UV\n"
	"	//    offset that scales with the ripple normal AND with view depth, so the\n"
	"	//    bottom appears to shimmer/displace under the moving surface (a cheap\n"
	"	//    refraction feel -- no second pass, no depth texture). Steeper grazing\n"
	"	//    views push the refraction offset further (longer path through water).\n"
	"	float viewDepth = max( dot( normalize( v_normal ), V ), 0.05 );\n"		// 1 top-down .. ~0 grazing
	"	float refrAmt = mix( 1.6, 0.4, viewDepth );\n"				// grazing refracts more
	...
	"	// reads as water rather than wet rock even from straight overhead. Tint\n"
	"	// deepens with view depth so the body color shifts toward teal where you\n"
	"	// look straight down into the water (more water column between eye and bed).\n"
	"	float bodyMix = mix( 0.34, 0.22, viewDepth );\n"
	"	vec3 body = mix( deep, tex * vec3( 0.40, 0.78, 0.92 ), bodyMix );\n"
	"\n"
