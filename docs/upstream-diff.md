...
| `cl_dll/environment.cpp` + `environment.h` + `hud.cpp` | ~12 | gl_snow_* cvars (FCVAR_ARCHIVE) used to register lazily in `CEnvironment::Initialize()` (first map load); the engine archives them into config.cfg on exit and every later boot execs that config before any map -> 5x "Unknown command" startup spam (M1 defect batch 3 #15). Registration moved to `CEnvironment::RegisterCvars()` (same file), called from `CHud::Init` before configs exec; `Initialize()` still calls it (upstream parity). Pure upstream-state fix, no cszrender logic involved. |
